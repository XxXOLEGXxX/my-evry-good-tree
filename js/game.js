...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		let gain = tmp[layer].baseAmount.div(tmp[layer].requires).sub(tmp[layer].gainAdd).div(tmp[layer].gainMult).max(1).log(tmp[layer].base).times(tmp[layer].gainExp).pow(Decimal.pow(tmp[layer].exponent, -1))
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		let extraCost = Decimal.pow(tmp[layer].base, amt.pow(tmp[layer].exponent).div(tmp[layer].gainExp)).times(tmp[layer].gainMult).add(tmp[layer].gainAdd)
